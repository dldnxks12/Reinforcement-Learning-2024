...
class InvertedTriplePendulumEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, action):
        ...
        dist_penalty = 0.01 * x ** 2 + (y - 2.5) ** 2

        v1, v2, v3 = self.data.qvel[1:4]

        vel_penalty = (1e-2 * v1**2) + (5e-3 * v2**2) + (5e-3 * v3**2)

        alive_bonus = 10
        r = alive_bonus - vel_penalty - dist_penalty
        if (ob[3] + ob[4] + ob[5]) > 2.7:
            r += 1
            if vel_penalty < 0.5:
                r = r * (1 + 0.1*(5 - 10 * vel_penalty))
                print(f"Additional reward Occured : {vel_penalty}")
                if ob[3] > 0.98 and vel_penalty < 0.05:
                    r = r * 1.5
    ...
    def reset_model(self):
        self.set_state(
            self.init_qpos + np.array([0,np.pi,0,0]) +
