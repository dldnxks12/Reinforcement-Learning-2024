...
class Trainer:
    ...
    def train(self):
        while not self.finish_flag:
            ...
            # # TODO : freeze 사용해서도 순서대로 처리되는지 확인
            # state_idx = []
                ...
                if self.local_step < self.init_obs_delayed_steps:  # if t < d
                    ...
                    if len(obs_temp) > 0 and self.state_tag == obs_temp[0][1]:
                        ...
                        if len(self.agent.temporary_buffer.states) >= (self.init_obs_delayed_steps + 2):
                            ...
                            self.agent.replay_memory.push(augmented_s, s, a, reward, next_augmented_s, next_s,
                                                          true_done)
                    ...
                    # 관측가능한 state도 없고, 처리할 state도 없을 때 -> no-ops
                        ...
                        if len(obs_temp) == 0:
                            ...
                            # print(f"Current tag {self.state_tag} | State idx {obs_temp[0][1]}")
                        print("------------------------- freeze occurred -------------------------")
    ...
    def evaluate(self):
        ...
        for epi in range(self.eval_episode):
            ...
            while not done:
                ...
                if self.eval_local_step < self.init_obs_delayed_steps:
                    ...
                    eval_obs_list, eval_reward_list, eval_done_list, info = self.eval_delayed_env.step(action)
                    ...
                    if len(eval_obs_temp) > 0:
                        # print(f"Current tag {self.eval_state_tag} | Ready to be processed state idx {eval_obs_temp[0][1]}")
                        if self.eval_state_tag == eval_obs_temp[0][1]:
                            next_state = eval_obs_temp[0][0]  # state
        ...
        log_to_txt(self.args.env_name, self.args.random_seed, self.init_obs_delayed_steps, self.min_obs_delayed_steps,
                   self.max_obs_delayed_steps, self.total_step, sum(reward_list) / len(reward_list), self.trial)
