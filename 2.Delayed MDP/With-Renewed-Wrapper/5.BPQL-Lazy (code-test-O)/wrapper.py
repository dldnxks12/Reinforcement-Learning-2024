...
class DelayedEnv(gym.Wrapper):
    ...
    def step(self, action, no_ops = False):
        ...
        if no_ops:
            return current_obs, current_reward, current_done
        ...
        for i, meta_obs in enumerate(self.obs_buffer):
            if meta_obs[1] + meta_obs[2] <= self.state_idx:  # induced time + delay <= current time
        ...
        # Pop the delayed obs, rwd, done
        # for meta_obs in ret_meta_obs_list:
        #     self.obs_buffer.remove(meta_obs)
        # for meta_reward in ret_meta_reward_list:
        #     self.reward_buffer.remove(meta_reward)
        # for meta_done in ret_meta_done_list:
        #     self.done_buffer.remove(meta_done)

        if self.mode == 'parallel':  # TCP communication
            return ret_meta_obs_list, ret_meta_reward_list, ret_meta_done_list, data_dict


        # elif self.mode == 'serial':  # Due to slow sensor algorithm
        #     ret_meta_obs_list    = sort_list(ret_meta_obs_list)
        #     ret_meta_reward_list = sort_list(ret_meta_reward_list)
        #     ret_meta_done_list   = sort_list(ret_meta_done_list)
        #
        #     ret_length = len(ret_meta_obs_list)
        #     if ret_length > 1:
        #         for i in range(1, ret_length):
        #             updated_meta_obs = ret_meta_obs_list[i]
        #             updated_meta_obs[2] += 1
        #             self.obs_buffer.append(updated_meta_obs)
        #
        #             updated_meta_reward = ret_meta_reward_list[i]
        #             updated_meta_reward[2] += 1
        #             self.reward_buffer.append(updated_meta_reward)
        #
        #             updated_meta_done = ret_meta_done_list[i]
        #             updated_meta_done[2] += 1
        #             self.delay_buffer.append(updated_meta_done)
        # else:
        #     raise NameError
