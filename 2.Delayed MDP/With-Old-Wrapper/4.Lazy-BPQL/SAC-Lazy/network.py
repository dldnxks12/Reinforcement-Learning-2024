...
class Twin_Q_net(nn.Module):
    def __init__(self, state_dim, delayed_steps, action_dim, device, hidden_dims=(256, 256), activation_fc=F.relu):
        super(Twin_Q_net, self).__init__()
        self.device = device

        self.activation_fc = activation_fc

        augmented_state_dim = state_dim + delayed_steps * action_dim
        self.input_layer_A = nn.Linear(augmented_state_dim + action_dim, hidden_dims[0])
        ...
        self.input_layer_B = nn.Linear(augmented_state_dim + action_dim, hidden_dims[0])
