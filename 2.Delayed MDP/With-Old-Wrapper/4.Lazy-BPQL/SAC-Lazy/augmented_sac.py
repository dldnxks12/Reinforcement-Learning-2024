...
class BPQLAgent:  # 5.SAC for the base learning algorithm
    def __init__(self, args, state_dim, action_dim, action_bound, action_space, device):
        ...
        # TODO : Critic Input Dim. : size of augmented state-space
        self.critic        = Twin_Q_net(state_dim, args.obs_delayed_steps + args.act_delayed_steps, action_dim, device, args.hidden_dims).to(device)  # Network for the beta Q-values.
        self.target_critic = Twin_Q_net(state_dim, args.obs_delayed_steps + args.act_delayed_steps, action_dim, device, args.hidden_dims).to(device)
