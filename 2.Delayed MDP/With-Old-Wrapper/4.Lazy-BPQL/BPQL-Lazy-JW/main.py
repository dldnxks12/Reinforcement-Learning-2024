...
if __name__ == '__main__':
    ...
    parser.add_argument('--obs-delayed-steps',     default=15, type=int)
    parser.add_argument('--max-obs-delayed-steps', default=15, type=int)
    ...
    for trial in range(0, 5):
        ...
        env, eval_env = make_delayed_env(args, random_seed, obs_delayed_steps=args.obs_delayed_steps, max_obs_delayed_steps=args.max_obs_delayed_steps)

        state_dim = env.observation_space.shape[0]
        action_dim = env.action_space.shape[0]
        action_bound = [env.action_space.low[0], env.action_space.high[0]]

        print(f"Environment: {args.env_name}, Init Obs. Delayed: {args.obs_delayed_steps}, Max-Obs. Delayed: {args.max_obs_delayed_steps}, Random Seed: {trial_seed}", "\n")
