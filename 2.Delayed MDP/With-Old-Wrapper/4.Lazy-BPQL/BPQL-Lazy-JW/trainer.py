...
class Trainer:
    def __init__(self, env, eval_env, agent, trial, args):
        ...
        self.obs_delayed_steps     = args.obs_delayed_steps
        ...
        """
            ...
            * obs_delayed_steps *
                ...
                - augmented state I의 차원은 obs_delayed_steps에 맞춰서 설정됩니다.
        
                - i.e.,  obs_delayed_steps = 3이라면, 
        
                    I = (s1, a1, a2, a3), I' = (s2, a2, a3, a4), I'' = (s3, a3, a4, a5), ...  
        
            # 참고. wrapper - init() , reset()
            ...
            * max_obs_delayed_steps *
                ...
                - Observation delay는 [1, O_max] 범위 안에서 uniform하게 샘플링 됩니다.  
        ...
        """
    def train(self):
        # TODO : Train Agent in constant delayed MDP where a constant delay o == obs_delayed_steps
        # TODO : Lazy-BPQL은 obs_delayed_steps == max_obs_delayed_steps 입니다.
        ...
        while not self.finish_flag:
            ...
            while not done:
                ...
                if self.local_step < self.obs_delayed_steps:  # if t < d
                    ...
                    first_action_idx = len(self.agent.temporary_buffer.actions) - self.obs_delayed_steps
                    ...
                    next_observed_state, reward, delay, done, info = self.delayed_env.step(action)
                    ...
                    if self.local_step > 2 * self.obs_delayed_steps:  # if t > 2d
    ...
    def evaluate(self):
        # TODO : Evaluate Agent in random delayed MDP : obs. delay o ~ [O_min, O_max]
        ...
        for epi in range(self.eval_episode):
            ...
            # TODO : state가 순서대로 처리되야하기 때문에 임시방편으로 state_tag 변수를 사용했습니다.
            # TODO : state_tag 변수는 state가 관측됨에 따라 1씩 증가합니다.
            state_tag = 1 # state tagging for ordering / s1 -> s2 -> s3 ...

            while not done:
                self.eval_local_step += 1

                if self.eval_local_step < self.obs_delayed_steps:
                ...
                elif self.eval_local_step == self.obs_delayed_steps:
                    ...
                    # TODO : state s_k는 t = k + o 이후 부터 관측 가능

                    # TODO : t = current time-step
                    # TODO : k = state가 induce된 time-step
                    # TODO : o = s_k의 delay
                    ...
                    if self.eval_local_step < last_observed_delay + state_tag:
                        ...
                        # TODO : Lazy-BPQL의 경우 항상 self.eval_local_step >= last_observed_delay + state_tag
                        # TODO : 가 되서 해당 if 문을 거치지 않습니다.
                        ...
                        first_action_idx = len(self.agent.eval_temporary_buffer.actions) - self.obs_delayed_steps
        ...
        log_to_txt(self.args.env_name, self.args.random_seed,  self.obs_delayed_steps, self.max_obs_delayed_steps, self.total_step, sum(reward_list) / len(reward_list), self.trial)
