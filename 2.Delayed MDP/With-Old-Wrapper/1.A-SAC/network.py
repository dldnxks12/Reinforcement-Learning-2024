...
class QNetwork(nn.Module): # Q(st, at)
...
class GaussianPolicy(nn.Module): # pi(-|It)
    def __init__(self, state_dim, action_dim, hidden_dim, action_bound):
        ...
        # action rescaling
        self.action_scale = torch.as_tensor((action_bound[1] - action_bound[0]) / 2., dtype=torch.float32)
        self.action_bias = torch.as_tensor((action_bound[1] + action_bound[0]) / 2., dtype=torch.float32)
