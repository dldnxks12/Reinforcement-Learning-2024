...
class DelayedEnv(gym.Wrapper):
    def __init__(self, env, seed, obs_delayed_steps, max_obs_delayed_steps):
        ...
        self.delayed_steps      = obs_delayed_steps
        self.max_delayed_steps  = max_obs_delayed_steps

    def reset(self):
        init_state, _ = self.env.reset()

        init_delay = random.randrange(1, self.max_delayed_steps + 1)

        for _ in range(self.delayed_steps):
    ...
    def step(self, action, no_ops = False):
        ...
        # TODO : delayed time-steps of current_obs
        current_delayed_time_steps = random.randrange(1, self.max_delayed_steps + 1)
