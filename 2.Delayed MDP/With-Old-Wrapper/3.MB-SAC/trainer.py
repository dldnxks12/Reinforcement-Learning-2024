        ...
        while not self.finish_flag:
            ...
            while not done:
                ...
                if self.agent.replay_memory.size >= self.batch_size and self.total_step >= self.update_after and \
                    ...
                    # Learn dynamics
                    self.agent.train_dynamics_model()
            ...
            while not done:
                ...
                if self.eval_local_step < self.delayed_steps:
                    ...
                    if self.eval_local_step < last_observed_delay + state_tag:
                        ...
                        # TODO : state 관측 가능 >> planning with dynamics model
                        last_observed_state = self.agent.eval_temporary_buffer.states[-1]
                        simulated_state     = self.agent.mpc(last_observed_state, last_observed_delay)
                        action = self.agent.get_action(simulated_state, evaluation=True)
