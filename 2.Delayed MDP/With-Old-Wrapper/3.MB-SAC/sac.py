...
class SAC:
    def __init__(self, args, state_dim, action_dim, action_bound, action_space, device):
        ...
        self.predictor = DynamicsModel(state_dim, action_dim, args.hidden_dims[0], self.device).to(device)
        self.predictor_optimizer =torch.optim.Adam(self.predictor.parameters(), lr=args.predictor_lr)
    ...
    def train_dynamics_model(self):
        ...
        for epoch in range(10):
            total_loss = 0.0
            actions, rewards, dones, states, next_states = self.replay_memory.sample(self.batch_size)
            for action, reward, done, state, next_state in zip(actions, rewards, dones, states, next_states):
                self.predictor_optimizer.zero_grad()
                predicted_next_state = self.predictor(state, action)
                model_loss = nn.MSELoss()(predicted_next_state, next_state)
                model_loss.backward()
            ...
            # if epoch % 10 == 0:
            #     print(f'Model Learning : Epoch {epoch}, Loss: {total_loss / len(actions)}')

    def mpc(self, state, delay):
        self.predictor.eval()
