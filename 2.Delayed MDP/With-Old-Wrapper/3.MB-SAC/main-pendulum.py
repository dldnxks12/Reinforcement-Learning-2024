...
if __name__ == '__main__':
    for trial in range(0, 5):
        ...
        parser.add_argument('--env-name', default='InvertedPendulum-v4', type=str)

        # TODO : Select augmented state dimension & O_max
        max_obs_delayed = 5
        obs_delay = random.randrange(1, max_obs_delayed+1)
        parser.add_argument('--obs-delayed-steps',     default=obs_delay, type=int)
        ...
        parser.add_argument('--predictor-lr', default=3e-4, type=float)
        ...
        # Create Undelayed Environment
        env, _ = make_env(args.env_name, random_seed)

        # Create Delayed Environment
        _ , eval_env = make_delayed_env(args, random_seed, obs_delayed_steps=args.obs_delayed_steps, max_obs_delayed_steps=args.max_obs_delayed_steps)

        state_dim     = env.observation_space.shape[0]
        action_dim    = env.action_space.shape[0]
        action_bound  = [env.action_space.low[0], env.action_space.high[0]]

        print(f"Environment: {args.env_name}, Max-Obs. Delayed: {args.max_obs_delayed_steps}, Random Seed: {trial_seed}", "\n")
